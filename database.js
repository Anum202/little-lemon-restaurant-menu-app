...
export function saveMenuItems(menuItems) {
  db.transaction((tx) => {
    ...
    console.log("Values saves:", values);

    tx.executeSql(sqlQuery, values, (_, result) => {
      if (result.rowsAffected > 0) {
        console.log(
          `Inserted ${result.rowsAffected} rows. Last inserted ID: ${result.insertId}`
        );
      } else {
...
export async function filterByQueryAndCategories(query, activeCategories) {
  return new Promise((resolve, reject) => {
    db.transaction((tx) => {
      ...
      // Check if there's a query string and add it to the WHERE clause
      if (query) {
        whereClause += ` AND title LIKE ?`;
        whereParams.push(`%${query}%`);
      }

      // Check if there are active categories and add them to the WHERE clause
      if (activeCategories.length > 0) {
        whereClause += ` AND category IN (${activeCategories
          .map(() => "?")
          .join(", ")})`;
        whereParams.push(...activeCategories);
      }
      ...
      tx.executeSql(sqlQuery, whereParams, (_, { rows }) => {
        resolve(rows._array);
      });
    });
  });
